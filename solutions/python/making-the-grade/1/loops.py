...
def round_scores(student_scores):
    ...
    # Tant que la liste student_scores n'est pas vide
    while student_scores:
        # On extrait la dernière note (ex: 40.3)
        note = student_scores.pop()
        # On l'arrondit et on l'ajoute à nos résultats
        rounded_score.append(round(note))
...
def letter_grades(highest):
    ...
    Returns:
        ...
        The result would be [41, 56, 71, 86]:
            ...
            86 <= "A" <= 100
    """
    grade_thresholds = (highest - 40) // 4

    return list(range(41, highest, grade_thresholds))
...
def student_ranking(student_scores, student_names):
    ...
    for index, name in enumerate(student_names): 
        sentence = f"{index + 1}. {name}: {student_scores[index]}"
        grade.append(sentence)
