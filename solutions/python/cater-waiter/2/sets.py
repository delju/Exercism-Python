...
def check_drinks(drink_name, drink_ingredients):
    ...
    if ingredients.intersection(ALCOHOLS):
...
def categorize_dish(dish_name, dish_ingredients):
    ...
    # Notre catalogue ordonné (Ensemble, "Nom de la catégorie")
    categories = [
        (VEGAN, "VEGAN"),
    ...
    # Ta boucle for magique qui teste tout d'un coup
    for reference_set, category_name in categories:
        if ingredients_set.issubset(reference_set):
...
def tag_special_ingredients(dish):
    ...
    allergen = ingredients.intersection(SPECIAL_INGREDIENTS)
...
def separate_appetizers(dishes, appetizers):
    ...
    """
    result = set(dishes) - set(appetizers)
...
def singleton_ingredients(dishes, intersection):
    ...
    all_ingredient = set().union(*dishes) 
    duplicates = set()
    already_seens = set()

    for ingredient in dishes: 
        duplicates.update(already_seens.intersection(ingredient))
        already_seens.update(ingredient)

    return all_ingredient - duplicates - intersection
