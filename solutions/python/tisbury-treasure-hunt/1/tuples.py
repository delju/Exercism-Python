...
def compare_records(azara_record, rui_record):
    ...
    """
    azara_coordinate = tuple(get_coordinate(azara_record))
...
def clean_up(combined_record_group):
    ...
    for item in combined_record_group:
        # On crée le tuple nettoyé sans l'index 1
        clean_record = (item[0], item[2], item[3], item[4])
        
        # On le convertit en texte et on l'ajoute à notre liste
        rapport_line.append(str(clean_record))
        
    # On relie toutes les lignes par un saut de ligne et on ajoute un '\n' tout à la fin
    return "\n".join(rapport_line) + "\n"
