...
def send_to_store(cart, aisle_mapping):
    ...
    # Exemple de tri par numéro d'allée (si shop_info est une liste [allée, frigo])
    # Si vous préférez garder votre tri alphabétique inversé actuel, remplacez par : 
    # key=lambda x: x[0], reverse=True
    sorted_items = sorted(
        result.items(), 
        key=lambda x: x[0], reverse=True  # Trie selon le premier élément de shop_info (l'allée)
    )
