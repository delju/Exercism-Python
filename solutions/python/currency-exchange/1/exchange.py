...
def exchangeable_value(budget, exchange_rate, spread, denomination):
    ...
    """
    # 1. On calcule le taux de change qui inclut les frais (spread)
    actual_rate = exchange_rate * (1 + spread / 100)
    
    # 2. On calcule la valeur totale convertie dans la nouvelle monnaie
    unrounded_value = budget / actual_rate
    
    # 3. On calcule combien de billets entiers on peut obtenir
    number_of_bills = unrounded_value // denomination
    
    # 4. On renvoie la valeur totale de ces billets (convertie en INT pour Exercism)
